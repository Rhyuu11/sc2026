...
function ovpn_install() {
    rm -rf /etc/openvpn
    mkdir -p /etc/openvpn
    wget -O /etc/openvpn/vpn.zip "https://raw.githubusercontent.com/Rhyuu11/sc2026/main/install/vpn.zip" >/dev/null 2>&1 
...
function config_easy() {
    ...
    systemctl restart openvpn-server@server-tcp openvpn-server@server-udp >/dev/null 2>&1 || true
}
